...
function ViewReports({ reportData, month, year }) {
  const [doc, setDoc] = React.useState(reportData);
  const [hydrating, setHydrating] = React.useState(false);
  const [cumTotals, setCumTotals] = React.useState({});
  const [cumFallback, setCumFallback] = React.useState(null);
  const flatRows = React.useMemo(() => buildFlatRows(sections), []);

  const id = reportData?._id || reportData?.id;
  const baseDistrict = reportData?.district || "";
  const baseInstitution = reportData?.institution || "";

  // Hydrate selected doc so the "During the Month" column is accurate
  React.useEffect(() => {
    if (!id) return;
    let cancelled = false;
    (async () => {
  ...
  // Ask backend for authoritative FY cumulative (April -> selected)
  React.useEffect(() => {
    setCumTotals({});
    ...
    (async () => {
      try {
        ...
        if (!cancelled && r.ok && j?.ok && j?.cumulative) setCumTotals(j.cumulative);
      } catch {}
    })();
    return () => { cancelled = true; };
  }, [baseDistrict, baseInstitution, month, year]);

  // Local FY fallback (latest doc per month in April -> selected window)
  React.useEffect(() => {
  ...
  const eyeBankData = fixNames(eyeBank || doc?.eyebank || doc?.eye_bank || []);
  const visionCenterData = fixNames(visionCenter || doc?.visioncentre || doc?.vision_centre || []);
  ...
  `;
{
  ...
  // 🧮 Local FY fallback (latest per month, April → selected), used only if server didn't provide totals
  useEffect(() => {
    if (cumTotals && Object.keys(cumTotals).length) { setCumFallback(null); return; }
    if (!baseDistrict || !baseInstitution || !month || !year) { setCumFallback(null); return; }
  ...
  // ✅ Choose cumulative source: server → local FY fallback → (optional) stored cumulative → {}
  const cumSrc =
    (cumTotals && Object.keys(cumTotals).length && cumTotals) ||
    (cumFallback && Object.keys(cumFallback).length && cumFallback) ||
    (cumulativeFromServer && Object.keys(cumulativeFromServer).length && cumulativeFromServer) ||
