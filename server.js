...
// Set this on Render: MONGO_URI = your MongoDB Atlas connection string
const MONGO_URI =
...
function _empty84() {
  return Array.from({ length: ALL_QUESTION_KEYS.length }, () => 0);
}
...
function _fiscalStartYear(month, year) {
  return ["January", "February", "March"].includes(month)
    ? Number(year) - 1
    : Number(year);
}
function _fiscalWindow(toMonth, toYear) {
  ...
  for (let i = 0; i < FISCAL_MONTHS.length; i++) {
    ...
    const y = i <= 8 ? startY : startY + 1; // Apr–Dec in startY, Jan–Mar in startY+1
...
app.use(
  cors({
    origin: "*", // loosen for public mobile clients; tighten later if needed
  })
);
...
/* Gate API routes until DB is ready (except health & preflight) */
...
/* Quick probe to examine a single question key across docs */
...
/* =======================
   District → Institution-wise report
   ======================= */
app.get("/api/district-institution-report", async (req, res) => {
  try {
    ...
    if (!district || !month || !year) {
      return res
        .status(400)
        .json({ ok: false, error: "Missing district/month/year" });
    }
    ...
    // pick latest per institution+month
    ...
    const institutionNames = Array.from(byInst.keys()).sort((a, b) =>
      a.localeCompare(b)
    );
    ...
    for (const inst of institutionNames) {
      ...
      // cumulative across fiscal window up to selected month
      let cum = _empty84();
      ...
      districtPerformance.cumulative = _sum84(
        districtPerformance.cumulative,
        cum
      );
    }
...
app.get("/api/report", async (req, res) => {
  try {
    ...
    if (!district || !institution || !month || !year) {
      return res
        .status(400)
        .json({ ok: false, error: "Missing district/institution/month/year" });
    }
...
/* Create or update with safe-merge support */
app.post("/api/reports", async (req, res) => {
  try {
    ...
    const merge =
      String(req.query.merge ?? "").toLowerCase() === "1" || req.body.merge;

    let prevDoc = null;
    if (merge) prevDoc = await Report.findOne(filter);

    // answers: prefer provided keys, otherwise keep existing (when merge)
    const baseAnswers = prevDoc ? _normalize84(prevDoc.answers) : _normalize84({});
    const incomingAnswers = _normalize84(answers);
    const finalAnswers = {};
    ...
    // cumulative: if provided, merge similarly; else mirror answers
    const baseCumulative = prevDoc
      ? _normalize84(prevDoc.cumulative || prevDoc.answers)
      : _normalize84({});
    const incomingCumulative = _normalize84(cumulative);
    const finalCumulative = {};
    for (const k of ALL_QUESTION_KEYS) {
      finalCumulative[k] = merge
        ? (cumulative[k] !== undefined
            ? incomingCumulative[k]
            : baseCumulative[k])
...
/* Simple login (password presence only; enforce on frontend if needed) */
app.post("/api/login", (req, res) => {
  try {
    const { district = "", institution = "", password = "" } = req.body || {};
    if (!district || !institution) {
      return res
        .status(400)
        .json({ ok: false, error: "Missing district/institution" });
    }
    if (!password) {
      return res.status(401).json({ ok: false, error: "Missing password" });
    }
    const role = institution.startsWith("DC ") ? "DOC" : "OPTOMETRIST";
    res.json({
      ok: true,
      user: { district, institution, role, isGuest: false },
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
...
mongoose
  .connect(MONGO_URI, {
    dbName: "optometry",
    // Optional: tune these for production
    // maxPoolSize: 10,
